...
get_balance:
	...
	cast call $(CONTRACT_ADDRESS) "balanceOf(address)(uint256)" $(SENDER_ADDRESS)

eth_balances:
	cast balance $(OWNER_ADDRESS)
	cast balance $(SENDER_ADDRESS)

eth_to_token:
	cast call $(CONTRACT_ADDRESS) "EthToToken(uint256)(uint256)" 10

buy:
	cast send $(CONTRACT_ADDRESS) "Buy(uint256)()" 10 --from $(SENDER_ADDRESS) --value 10ether
